...
Архитектура:
  1) IntentRouter — крошечный промпт. Только выбирает один из списка интентов.
...
15) set_category — назначить/сменить категорию карточки.
   Категории v1: home, work, interests, routine, study, health.
   Примеры: "поставь категорию work", "это домашние дела", "пометь как study".

16) create_routine_template — создать/обновить шаблон ежедневной рутины.
   Примеры: "каждый день английский на 60 минут", "создай рутину утренней зарядки".

17) pause_routine_template — поставить рутину на паузу.
   Примеры: "останови рутину английский", "пауза рутина зарядка".

18) resume_routine_template — возобновить рутину.
   Примеры: "возобнови рутину английский", "включи рутину зарядка".

19) list_routine_templates — показать список рутин.
   Примеры: "покажи рутины", "список рутин".

20) set_persona — переключить режим: "будь как мама/илон/дзен", "переключись на zen".

21) link_board — пользователь привязывает доску Trello (присылает ссылку https://trello.com/b/...).

22) chitchat — нет конкретного действия. Приветствие, благодарность, общий вопрос.
...
Если карточка явно не указана — оставь card_id/card_name null.
""",
...
- routine_name: название шаблона.
- routine_duration_min: минуты, только если пользователь назвал длительность.
- category_key: из {home,work,interests,routine,study,health}, если указано.
- checklist_item: пункты через ";" если пользователь перечислил шаги.
""",
